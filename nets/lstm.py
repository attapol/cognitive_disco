...
class BinaryTreeLSTM(LSTM):
    ...
    def __init__(self, rng, dim_proj, n_out=None, W=None, U=None, b=None):
        ...
        c_matrix = T.tensor3('x', dtype=config.floatX) 

        word_mask_matrix = T.matrix(dtype=config.floatX)
        c_mask_matrix = T.matrix(dtype=config.floatX)
        children = T.lmatrix()

        self.input = [X, c_matrix, children, word_mask_matrix, c_mask_matrix]
        n_samples = X.shape[1]

        self.h = self.project(X, c_matrix, children, 
                word_mask_matrix, c_mask_matrix)
        self.top_h = self.h[c_mask_matrix.sum(axis=0).astype('int64') - 1,
                T.arange(n_samples), :]
    ...
    def project(self, hidden, c_matrix, children, 
            word_mask_matrix, c_mask_matrix):
        """

        hidden - (shared) TxNxd serrated tensor prefilled with word embeddings for leaves
        c_matrix - (shared) TxNxd serrated tensor prefilled with zeros
        children - TxNx2 serrated matrix for children list 

        word_mask_matrix - TxN mask for varying leaves list
        c_mask_matrix - TxN mask for varying children list

        """
        nsteps = hidden.shape[0]
        if hidden.ndim == 3:
            n_samples = hidden.shape[1]
        else:
            n_samples = 1

        step_idx = T.lscalar()

        left_child_idx = children[step_idx][0]
        right_child_idx = children[step_idx][1]
        child_mask = c_mask_matrix[step_idx]
        x_mask = word_mask_matrix[step_idx]
        ...
        recursive = \
                ...
                self.b
        embedding = hidden[step_idx, T.arange(n_samples), :] # from embedding 
        preact = x_mask[:, None] * embedding + (1. - x_mask) * recursive
        ...
        new_c = i * c_prime + f1 * c_matrix[left_child_idx] +\
                f2 * c_matrix[right_child_idx]
        new_c_masked = child_mask[:, None] * new_c +\
                (1. - child_mask[:, None]) * c_matrix[step_idx]
        new_h = o * T.tanh(new_c_masked)
        new_h_masked = child_mask[:,None] * new_h +\
                (1. - child_mask[:, None]) * hidden[step_idx]

        #updating 
        stepping = theano.function(
                inputs=[step_idx], 
                updates=[
                    (c_matrix, T.set_subtensor(
                        c_matrix[step_idx], new_c_masked)), 
                    (hidden, T.set_subtensor(
                        hidden[step_idx], new_h_masked))])
    ...
    def make_givens(givens, input_vec, T_training_data, start_idx, end_idx):
        ...
        givens[input_vec[4]] = T_training_data[4][:,start_idx:end_idx]

        givens[input_vec[0+5]] = T_training_data[0+5][:,start_idx:end_idx, :]
        givens[input_vec[1+5]] = T_training_data[1+5][:,start_idx:end_idx, :]
        givens[input_vec[2+5]] = T_training_data[2+5][:,start_idx:end_idx, :]
        givens[input_vec[3+5]] = T_training_data[3+5][:,start_idx:end_idx]
        givens[input_vec[4+5]] = T_training_data[4+5][:,start_idx:end_idx]
...
def prep_tree_srm_arg(relation_list, arg_pos, wbm, max_length, parses):
    assert arg_pos == 1 or arg_pos == 2
    n_samples = len(relation_list)
    c_matrix = np.zeros((max_length, n_samples, wbm.num_units),
            dtype=config.floatX)
    ...
    for i, relation in enumerate(relation_list):
        ...
        parse_tree = tree_util.find_parse_tree(relation, arg_pos, parses)
        ordering_matrix = tree_util.reverse_toposort(parse_tree)

        num_nodes = min(max_length, ordering_matrix.shape[0])
        c_mask[:num_nodes, i] = 1.
        children[i, :num_nodes, :] = ordering_matrix[:num_nodes, 1:]
    np.swapaxes(children, 0, 1)
    return c_matrix, children, c_mask
...
def prep_tree_lstm_serrated_matrix_relations(relation_list, parses, 
        wbm, max_length):
    arg1_srm, arg1_mask = prep_srm_arg(relation_list, 1, wbm, max_length)
    arg2_srm, arg2_mask = prep_srm_arg(relation_list, 2, wbm, max_length)
    arg1_c_matrix, arg1_children, arg1_c_mask = \
            prep_tree_srm_arg(relation_list, 1, wbm, max_length, parses)
    arg2_c_matrix, arg2_children, arg2_c_mask = \
            prep_tree_srm_arg(relation_list, 2, wbm, max_length, parses)
    return (arg1_srm, arg1_c_matrix, arg1_children, arg1_mask, arg1_c_mask, \
            arg2_srm, arg2_c_matrix, arg2_children, arg2_mask, arg2_c_mask)
